...
Entering the numbers found in table \ref{tab:analysis_data} (p. \pageref{tab:analysis_data}) gives us a value of 95.37 frames per second. Given that HDMI has a 60 Hz refresh rate this system satisfies that constraint.
...
Using formulas \ref{eq:frametime} and \ref{eq:fps} and the values found in 
\ref{tab:analysis_data} (p. \pageref{tab:analysis_data}), a performance of 6,88 FPS can be calculated. 
...
The Maximum throughput of the TRD is 95.37 frames per second. A frame consists of  $1920\times1080$ pixels  with 32 bpp. This means that each pixel is represented by 4 bytes. This gives a size of 8.29 MB for one frame. This gives a throughput of 791 MB/s. As is demonstrated in table \ref{tab:bwscale} the system throughput is always lower than the available memory bandwidth.
